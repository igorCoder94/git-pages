      ...
      <div className="news-cards__content">
        ...
        {cardData.content.map((content, index) => {
          return (
            <div key={index}>
              {content.text ? (
                <div className="news-cards__text">{content.text}</div>
              ) : null}
...
const NewsCards = () => {
  ...
  return (
    <section className="news-cards__section">
      <div className="news-cards__container container">
        <div className="news-cards__blocks">
          {newsData.map((cardData, index) => {
            if (cardsCount > index) {
              return <NewsCard cardData={cardData} key={cardData.id} />;
            } else {
              return null;
            }
          })}
        </div>
        {cardsCount < newsData.length ? (
          <div
            className="news-cards__button"
            onClick={() => onLoadMore(cardsCount)}
          >
            Показать ещё
          </div>
        ) : null}
